...
bool computeVisibilityPolygon(const PolygonWithHoles& pwh,
  ...
  if (!pointInPolygon(pwh, query_point)) {
    RCLCPP_ERROR(rclcpp::get_logger("computeVisibilityPolygon"), "Query point outside of polygon.");
  }
  if (!isStrictlySimple(pwh)) {
    RCLCPP_ERROR(rclcpp::get_logger("computeVisibilityPolygon"), "Polygon is not strictly simple.");
  }
  ...
  if (poly.number_of_unbounded_faces() != 1) {
    RCLCPP_ERROR(rclcpp::get_logger("computeVisibilityPolygon"), "Polygon has unbounded curves.");
  }
  if (poly.number_of_faces() != 2) {
    RCLCPP_ERROR(rclcpp::get_logger("computeVisibilityPolygon"), "More than one bounded face in polygon.");
  }
  ...
  if ((f = boost::get<VisibilityArrangement::Face_const_handle>(&pl_result))) {
    ...
    while ((he->target()->point() != (*v)->point()) ||
      ...
      if (he == poly.halfedges_end()) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("computeVisibilityPolygon"),"Cannot find halfedge corresponding to vertex.");
        return false;
      }
    }
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("computeVisibilityPolygon"),"Cannot locate query point on arrangement.");
    return false;
  }

  // Result assertion.
  if (fh->is_fictitious()) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("computeVisibilityPolygon"),"Visibility polygon is fictitious.");
    return false;
  }
  if (fh->is_unbounded()) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("computeVisibilityPolygon"),"Visibility polygon is unbounded.");
    return false;
  }
