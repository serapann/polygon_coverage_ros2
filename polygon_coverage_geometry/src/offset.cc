...
// void computeOffsetPolygon( const PolygonWithHoles& pwh, const FT max_offset,
//                           PolygonWithHoles* offset_polygon) {
  assert(offset_polygon);

  PolygonWithHoles sorted_pwh = pwh;
  // const PolygonWithHoles sorted_pwh = pwh;
  ...
  rclcpp::Logger logger = rclcpp::get_logger("computeOffsetPolygon"); 
...
bool offsetEdge(const Polygon_2& poly, const size_t& edge_id, double offset,
  ...
  rclcpp::Logger logger = rclcpp::get_logger("offsetEdge"); 
  assert(offset_polygon);
  *offset_polygon = poly;

  if (!poly.is_simple()) {
    //ROS_WARN_STREAM("Polygon is not simple.");
    RCLCPP_WARN_STREAM(rclcpp::get_logger("offsetEdge"), "Polygon is not simple.");
    return false;
  }
  ...
  if (0.5 * polygon.bbox().ymax() <= max_y) {
    ...
    RCLCPP_WARN_STREAM(rclcpp::get_logger("offsetEdge"),"Offset too large. Re-adjusting.");
  }
  ...
  if (intersection_list.size() != 1) {
    ...
    RCLCPP_WARN_STREAM(logger,"Not exactly one resulting intersections.");
    RCLCPP_WARN_STREAM(logger,"Polygon: " << polygon);
    RCLCPP_WARN_STREAM(logger,"Mask: " << mask);
    RCLCPP_WARN_STREAM(logger,"Intersections:");
    for (auto p : intersection_list) 
      //ROS_WARN_STREAM(p << "\n");
      RCLCPP_WARN_STREAM(logger, p << "\n");
    return false;
  }
  if (intersection_list[0].number_of_holes() > 0) {
    //ROS_WARN_STREAM("Mask intersection has holes.");
    RCLCPP_WARN_STREAM(logger,"Mask intersection has holes.");
    return false;
  }
  ...
  if (diff_list.size() != 1) {
    /*ROS_WARN_STREAM("Not exactly one resulting difference polygon."
                    << diff_list.size());*/
    RCLCPP_WARN_STREAM(logger,"Not exactly one resulting difference polygon."
                    << diff_list.size());
    return false;
  }
  if (diff_list[0].number_of_holes() > 0) {
    //ROS_WARN_STREAM("Polygon difference has holes.");
    RCLCPP_WARN_STREAM(logger,"Polygon difference has holes.");
    return false;
  }
