...
bool computeSweep(const Polygon_2& in,
  ...
  while (has_sweep_segment) {
    ...
    if (!has_sweep_segment &&
      ...
      has_sweep_segment = findSweepSegment(in, sweep, &sweep_segment);
      if (!has_sweep_segment) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("computeSweep"),"Failed to calculate final sweep.");
        return false;
      }
    ...
    if (has_sweep_segment) {
      ...
      if (unobservable_point != sorted_pts.end()) {
        ...
        has_sweep_segment = findSweepSegment(in, sweep, &sweep_segment);
        if (!has_sweep_segment) {
          RCLCPP_ERROR_STREAM(rclcpp::get_logger("computeSweep"),"Failed to calculate extra sweep at point: "
                           << *unobservable_point);
          return false;
        }
      }
...
bool computeAllSweeps(const Polygon_2& poly, const double max_sweep_offset,
  ...
  for (const Direction_2& dir : dirs) {
    ...
    if (!computeSweep(poly, vis_graph, max_sweep_offset, dir, counter_clockwise,
                      &sweep)) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("computeAllSweeps"),"Cannot compute counter-clockwise sweep.");
      return false;
    } else {
      assert(!sweep.empty());
    ...
    if (!computeSweep(poly, vis_graph, max_sweep_offset, dir,
      ...
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("computeAllSweeps"),"Cannot compute clockwise sweep.");
      return false;
    } else {
      assert(!sweep.empty());
      cluster_sweeps->push_back(sweep);
...
bool calculateShortestPath(
  ...
  if (!computeVisibilityPolygon(visibility_graph.getPolygon(), start,
                                &start_visibility)) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("calculateShortestPath"),"Cannot compute visibility polygon from start query point "
                     << start
  ...
  if (!computeVisibilityPolygon(visibility_graph.getPolygon(), goal,
                                &goal_visibility)) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("calculateShortestPath"),"Cannot compute visibility polygon from goal query point "
                     << goal
                     << " in polygon: " << visibility_graph.getPolygon());
    return false;
  }
  if (!visibility_graph.solve(start, start_visibility, goal, goal_visibility,
                              shortest_path)) {
    /*RCLCPP_ERROR_STREAM(rclcpp::get_logger("calculateShortestPath"),"Cannot compute shortest path from "
                     << start << " to " << goal
                     << " in polygon: " << visibility_graph.getPolygon());*/
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("calculateShortestPath"), "Cannot compute shortest path from "
                     << start << " to " << goal
  ...
  if (shortest_path->size() < 2) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("calculateShortestPath"),"Shortest path too short.");
    return false;
  }
