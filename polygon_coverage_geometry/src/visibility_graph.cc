...
bool VisibilityGraph::create() {
  ...
  for (const VertexConstCirculator& v : graph_vertices) {
    ...
    if (!computeVisibilityPolygon(polygon_, *v, &visibility)) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("VisibilityGraph_create"),"Cannot compute visibility polygon.");
      return false;
    }
  ...
  RCLCPP_ERROR_STREAM(rclcpp::get_logger("VisibilityGraph_create"),"Created visibility graph with "
...
bool VisibilityGraph::addEdges() {
  if (graph_.empty()) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("VisibilityGraph_addEdges"),"Cannot add edges to an empty graph.");
    return false;
  }
  ...
  for (size_t adj_id = 0; adj_id < new_id; ++adj_id) {
    ...
    if (adj_node_property == nullptr) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("VisibilityGraph_addEdges"),"Cannot access potential neighbor.");
      return false;
    }
...
bool VisibilityGraph::solve(const Point_2& start,
  ...
  if (!is_created_) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("VisibilityGraph_solve"),"Visibility graph not initialized.");
    return false;
  } else if (!pointInPolygon(polygon_, start) ||
             !pointInPolygon(polygon_, goal)) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("VisibilityGraph_solve"),"Start or goal is not in polygon.");
    return false;
  }
  ...
  if (!temp_visibility_graph.solveAStar(start_idx, goal_idx, &solution)) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("VisibilityGraph_solve"),
        "Could not find shortest path. GrBUILD_SHARED_LIBSaph not fully connected.");
    return false;
  }
...
bool VisibilityGraph::getWaypoints(const Solution& solution,
  ...
  for (size_t i = 0; i < solution.size(); i++) {
    ...
    if (node_property == nullptr) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("VisibilityGraph_getWaypoints"),"Cannot reconstruct solution.");
      return false;
    }
...
bool VisibilityGraph::calculateHeuristic(size_t goal,
  ...
  if (goal_node_property == nullptr) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("VisibilityGraph_calculateHeuristic"),"Cannot find goal node property to calculate heuristic.");
    return false;
  }
  ...
  for (size_t adj_id = 0; adj_id < graph_.size(); ++adj_id) {
    const NodeProperty* adj_node_property = getNodeProperty(adj_id);
    if (adj_node_property == nullptr) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("VisibilityGraph_calculateHeuristic"),
