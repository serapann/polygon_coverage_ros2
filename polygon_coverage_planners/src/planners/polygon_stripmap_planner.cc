...
bool PolygonStripmapPlanner::setup() {
  ...
  if (is_initialized_) {
    RCLCPP_INFO(rclcpp::get_logger("PolygonStripmapPlanner::setup"),"Start creating sweep plan graph.");
    sweep_plan_graph_ = sweep_plan_graph::SweepPlanGraph(settings_);
    if (!sweep_plan_graph_.isInitialized()) {
      RCLCPP_ERROR(rclcpp::get_logger("PolygonStripmapPlanner::setup"),"Cannot create sweep plan graph.");
...
bool PolygonStripmapPlanner::solve(const Point_2& start, const Point_2& goal,
 ...
 assert(solution);
  ...
  if (!is_initialized_) {
    RCLCPP_ERROR(rclcpp::get_logger("PolygonStripmapPlanner::solve"),
  ...
  if (!runSolver(start_new, goal_new, solution)) {
    RCLCPP_ERROR(rclcpp::get_logger("PolygonStripmapPlanner::solve"),"Failed solving graph.");
    return false;
  }
...
bool PolygonStripmapPlanner::runSolver(const Point_2& start,
 ...
 assert(solution);
  ...
  RCLCPP_INFO(rclcpp::get_logger("PolygonStripmapPlanner::runSolver"),"Start solving GTSP using GK MA.");
