...
bool NodeProperty::isNonOptimal(
  ...
  if (waypoints.empty()) {
    RCLCPP_WARN_STREAM(rclcpp::get_logger("NodeProperty::isNonOptimal"),"Node does not have waypoints.");
    return false;
  }

  for (const NodeProperty& node_property : node_properties) {
    if (node_property.waypoints.empty()) {
      RCLCPP_WARN_STREAM(rclcpp::get_logger("NodeProperty::isNonOptimal"),"Comparison node does not have waypoints.");
      continue;
    }
...
bool SweepPlanGraph::create() {
  ...
  if (!computeDecomposition()) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::create"),"Failed to compute decomposition.");
    return false;
  }
  if (!offsetDecomposition()) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::create"),"Failed to offset neighboring decomposition cells.");
    return false;
  }
  ...
  for (size_t cluster = 0; cluster < polygon_clusters_.size(); ++cluster) {
    ...
    if (settings_.sweep_single_direction) {
      ...
      if (!computeSweep(polygon_clusters_[cluster], vis_graph,
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::create"),
      ...
      if (!computeAllSweeps(polygon_clusters_[cluster],
                            ...
                            &cluster_sweeps)) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::create"),"Cannot create all sweep plans for cluster "
                         << cluster);
        return false;
      }
    }
   ...
   RCLCPP_INFO_STREAM(rclcpp::get_logger("SweepPlanGraph::create"),"Created sweep plan graph with "
                  << graph_.size() << " nodes and " << edge_properties_.size()
                  << " edges.");
   RCLCPP_INFO_STREAM(rclcpp::get_logger("SweepPlanGraph::create"),"Initially created " << num_sweep_plans << " nodes.");
   RCLCPP_INFO_STREAM(rclcpp::get_logger("SweepPlanGraph::create"),"Pruned " << num_sweep_plans - graph_.size() << " nodes.");
  is_created_ = true;
  return true;
}
  ...
  switch (settings_.decomposition_type) {
    case DecompositionType::kBCD: {
      if (!computeBestBCDFromPolygonWithHoles(settings_.polygon,
                                              &polygon_clusters_)) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::computeDecomposition"),"Cannot compute boustrophedon decomposition.");
        return false;
      } else {
         RCLCPP_INFO_STREAM(rclcpp::get_logger("your_logger_name"),"Successfully created boustrophedon decomposition with "
    ...
    case DecompositionType::kTCD: {
      if (!computeBestTCDFromPolygonWithHoles(settings_.polygon,
                                              &polygon_clusters_)) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::computeDecomposition"),"Cannot compute trapezoidal decomposition.");
        return false;
      } else {
         RCLCPP_INFO_STREAM(rclcpp::get_logger("your_logger_name"),"Successfully created trapezoidal decomposition with "
                        << polygon_clusters_.size() << " polygon(s).");
      }
      break;
    }
    default: {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::computeDecomposition"),"No valid decomposition type set.");
      return false;
      break;
    }
  }
...
bool SweepPlanGraph::offsetDecomposition() {
  ...
  if (settings_.offset_polygons && !calculateDecompositionAdjacency(&adj)) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::offsetDecomposition"),"Decomposition not fully connected.");
    return false;
  }
  timer_poly_adj.Stop();
  ...
  if (settings_.offset_polygons && !offsetAdjacentCells(adj)) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::offsetDecomposition"),"Failed to offset rectangular decomposition.");
    return false;
  }
          ...
          if (result) {
            if (const Segment_2* s = boost::get<Segment_2>(&*result)) {
              if (*s == cell.edge(cell_e) ||
                ...
                RCLCPP_DEBUG_STREAM(rclcpp::get_logger("SweepPlanGraph::offsetAdjacentCells"),"Segment intersection but not identical.");
              }
            } else {
              RCLCPP_DEBUG_STREAM(rclcpp::get_logger("SweepPlanGraph::offsetAdjacentCells"),"Only point intersection... "
...
bool SweepPlanGraph::createNodeProperty(size_t cluster,
  ...
  if (!computeStartAndGoalVisibility(settings_.polygon, waypoints,
                                     &visibility_polygons)) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::createNodeProperty"),"Cannot compute start and goal visibility graph.");
    return false;
  }
...
bool SweepPlanGraph::addEdges() {
  if (graph_.empty()) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::addEdges"),"Cannot add edges to an empty graph.");
    return false;
  }
...
bool SweepPlanGraph::computeEdge(const EdgeId& edge_id,
  ...
  if (from_node_property->waypoints.empty() ||
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::computeEdge"),"Waypoints in node property are empty.");
    return false;
  }
  ...
  if (!visibility_graph_.solve(from_node_property->waypoints.back(),
                               ...
                               &shortest_path)) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::computeEdge"),"Cannot compute shortest path from "
...
bool SweepPlanGraph::solve(const Point_2& start, const Point_2& goal,
  ...
  if (!is_created_) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::solve"),"Graph not created.");
    return false;
  }
  ...
  if (!temp_gtsp_graph.addStartNode(start_node) ||
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::solve"),"Cannot add start and goal.");
    return false;
  }
  ...
  if (!temp_gtsp_graph.getClusters(&clusters)) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::solve"),"Cannot get clusters.");
    return false;
  }
  ...
  RCLCPP_INFO(rclcpp::get_logger("SweepPlanGraph::solve"),"Start solving GTSP");
  if (!solver.solve()) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::solve"),"GkMa solution failed.");
    return false;
  }
  RCLCPP_INFO(rclcpp::get_logger("SweepPlanGraph::solve"),"Finished solving GTSP");
  ...
  if (start_it == solution.end()) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::solve"),"Cannot find start node in solution.");
    return false;
  }
  std::rotate(solution.begin(), start_it, solution.end());
  if (solution.back() != goal_idx) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::solve"),"Goal node is not at back of solution.");
    return false;
  }

  if (!temp_gtsp_graph.getWaypoints(solution, waypoints)) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("SweepPlanGraph::solve"),"Cannot recover waypoints.");
    return false;
  }
