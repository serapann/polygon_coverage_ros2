...
bool GtsppProductGraph::create() {
  if (sweep_plan_graph_ == nullptr || boolean_lattice_ == nullptr) {
    RCLCPP_ERROR(rclcpp::get_logger("GtsppProductGraph::create"),"Sweep plan graph or boolean lattice not set.");
    return false;
  }
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("GtsppProductGraph::create"),"Created GTSPP product graph with "
...
bool GtsppProductGraph::createOnline() {
  if (sweep_plan_graph_ == nullptr || boolean_lattice_ == nullptr) {
    RCLCPP_ERROR(rclcpp::get_logger("GtsppProductGraph::createOnline"),"Sweep plan graph or boolean lattice not set.");
    return false;
  }
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("GtsppProductGraph::createOnline"),"Created GTSPP product graph with "
...
bool GtsppProductGraph::addStartNode(const NodeProperty& node_property) {
  ...
  for (size_t lattice_id = 0; lattice_id < boolean_lattice_->size();
    ...
    if (elapsed.count() > kTimeOut) {
      RCLCPP_ERROR(rclcpp::get_logger("GtsppProductGraph::addStartNode"),"Timout addStartNode.");
      return false;
    }
...
bool GtsppProductGraph::addGoalNode(const NodeProperty& node_property) {
  ...
  for (size_t lattice_id = 0; lattice_id < boolean_lattice_->size();
    ...
    std::chrono::duration<double> elapsed = current_time - start_time;
    if (elapsed.count() > kTimeOut) {
      RCLCPP_ERROR(rclcpp::get_logger("GtsppProductGraph::addGoalNode"),"Timout addGoalNode.");
      return false;
    }
...
bool GtsppProductGraph::solve(const Point_2& start, const Point_2& goal,
  ...
  if (!temp_gtspp_product_graph.solveDijkstra(&solution)) {
    RCLCPP_ERROR(rclcpp::get_logger("GtsppProductGraph::solve"),"Dijkstra failed.");
    return false;
  }
...
bool GtsppProductGraph::solveOnline(const Point_2& start, const Point_2& goal,
  ...
  if (!temp_gtspp_product_graph.createDijkstra(&solution)) {
    RCLCPP_ERROR(rclcpp::get_logger("GtsppProductGraph::solveOnline"),"Dijkstra failed.");
    return false;
  }
...
bool GtsppProductGraph::getWaypoints(const Solution& solution,
  ...
  if (sweep_plan_graph_ == nullptr) {
    RCLCPP_ERROR(rclcpp::get_logger("GtsppProductGraph::getWaypoints"),"Sweep plan graph not set.");
    return false;
  }
...
bool GtsppProductGraph::addEdges() {
  ...
  if (graph_.empty()) {
    RCLCPP_ERROR(rclcpp::get_logger("GtsppProductGraph::addEdges"),"Cannot add edges to an empty graph.");
    return false;
  }
...
bool GtsppProductGraph::createDijkstra(Solution* solution) {
  ...
  while (!open_set.empty()) {
    ...
    std::chrono::duration<double> elapsed = current_time - start_time;
    if (elapsed.count() > kTimeOut) {
      RCLCPP_ERROR(rclcpp::get_logger("GtsppProductGraph::createDijkstra"),"Timout createDijkstra.");
      return false;
    }
