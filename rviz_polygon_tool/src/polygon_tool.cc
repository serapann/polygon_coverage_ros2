...
void PolygonTool::onInitialize() {
  ...
  sphere_ =
      new rviz_rendering::Shape(rviz_rendering::Shape::Sphere, scene_manager_, moving_vertex_node_);
  ...
  //     "polygon", 1, true);
  polygon_pub_ = nh_->create_publisher<polygon_coverage_msgs::msg::PolygonWithHolesStamped>("polygon", 1);
...
bool rayIntersectsPlane(const Ogre::Ray& ray, const Ogre::Plane& plane, Ogre::Vector3& intersection) {
    Ogre::Real denom = plane.normal.dotProduct(ray.getDirection());
    ...
    Ogre::Real t = -(ray.getOrigin().dotProduct(plane.normal) + plane.d) / denom;
    if (t < 0) {
        return false;  // 交点在射线方向上的负方向，不在平面上
    }
...
bool getPointOnPlaneFromWindowXY(rviz_common::RenderPanel* panel, Ogre::Plane& plane, 
                        int window_x, int window_y, Ogre::Vector3& intersection_out) {
  int width = rviz_rendering::RenderWindowOgreAdapter::getOgreViewport(panel->getRenderWindow())->getActualWidth();
  int height = rviz_rendering::RenderWindowOgreAdapter::getOgreViewport(panel->getRenderWindow())->getActualHeight();
 
  Ogre::Ray mouse_ray = rviz_rendering::RenderWindowOgreAdapter::getOgreCamera(panel->getRenderWindow())->getCameraToViewportRay( (float)window_x / (float)width,
...
int PolygonTool::processMouseEvent(rviz_common::ViewportMouseEvent& event) {
  ...
  if (getPointOnPlaneFromWindowXY(event.panel, polygon_plane, event.x,
...
void PolygonTool::renderPolygon(const Polygon_2& polygon,
  ...
  for (auto v = polygon.vertices_begin(); v != polygon.vertices_end(); ++v) {
    ...
    rviz_rendering::Shape* sphere =
        new rviz_rendering::Shape(rviz_rendering::Shape::Sphere, scene_manager_, polygon_node_);
    sphere->setColor(c);
...
void PolygonTool::publishPolygon() {
  ...
  for (auto p = polygons_.begin(); p != polygons_.end(); ++p) {
    if (!p->is_simple()) {
      RCLCPP_WARN(rclcpp::get_logger("PolygonTool::publishPolygon"),"All polygons need to be simple!");
      return;
    }
  }
  ...
  if (res.size() > 1) {
    RCLCPP_WARN(rclcpp::get_logger("PolygonTool::publishPolygon"),"Polygon union results in more than one polygon. Remove holes.");
    return;
  }
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("PolygonTool::publishPolygon"),"Publishing polygon: " << res.front());
}
...
void PolygonTool::increaseAltitude(rviz_common::ViewportMouseEvent& event) {
  if (event.shift()) {
...
void PolygonTool::decreaseAltitude(rviz_common::ViewportMouseEvent& event) {
  if (event.shift()) {
    altitude_ -= kSmallAltitudeDelta;
  } else if (event.control()) {
    altitude_ -= kLargeAltitudeDelta;
