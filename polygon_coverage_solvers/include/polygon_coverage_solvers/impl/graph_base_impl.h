...
bool GraphBase<NodeProperty, EdgeProperty>::addStartNode(
  ...
  if (addNode(node_property)) {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("graphbase_addstartnode"), "Failed adding start node.");
    return false;
  }
}
...
bool GraphBase<NodeProperty, EdgeProperty>::addGoalNode(
  ...
  if (addNode(node_property)) {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("graphbase_addgoalnode"), "Failed adding goal node.");
    return false;
  }
}
  ...
  if (edgeExists(edge_id)) {
    *cost = graph_.at(edge_id.first).at(edge_id.second);
    ...
    /*ROS_ERROR_STREAM("Edge from " << edge_id.first << " to " << edge_id.second
                                  << " does not exist.");*/
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("graphbase_getedgecost"), "Edge from " << edge_id.first << " to " << edge_id.second << " does not exist.");
    *cost = -1.0;
    return false;
  }
}
...
const NodeProperty* GraphBase<NodeProperty, EdgeProperty>::getNodeProperty(
  ...
  if (nodePropertyExists(node_id)) {
   ...
   // ROS_ERROR_STREAM("Cannot access node property " << node_id << ".");
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("graphbase_getnodeproperty"), "Cannot access node property " << node_id << ".");
    return nullptr;
  }
}
...
GraphBase<NodeProperty, EdgeProperty>::GraphBase::getEdgeProperty(
  ...
  if (edgePropertyExists(edge_id)) {
    ...
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("graphbase_getedgeproperty"), "Cannot access edge property from " << edge_id.first << " to " << edge_id.second << ".");
    return nullptr;
  }
}
...
bool GraphBase<NodeProperty, EdgeProperty>::calculateHeuristic(
  ...
  RCLCPP_ERROR(rclcpp::get_logger("graphbase_calculateheuristic"), "Heuristic not implemented.");
  return false;
}
...
GraphBase<NodeProperty, EdgeProperty>::getAdjacencyMatrix() const {
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("graphbase_getAdjacencyMatrix"), "Create adjacency matrix.");
  ...
  if (sorted_cost.back() == 0.0) {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("graphbase_getAdjacencyMatrix"),"Adjacency matrix invalid. Greatest cost is 0.0.");
  ...
  if (max_scale * total_cost_upper_bound < std::numeric_limits<int>::max()) {
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("graphbase_getAdjacencyMatrix"), "Optimal scale %.9f applied.", max_scale);
    scale = max_scale;
  } else {
    //ROS_DEBUG("Minimum scale %.9f applied.", min_scale);
    RCLCPP_DEBUG(rclcpp::get_logger("graphbase_getAdjacencyMatrix"), "Minimum scale %.9f applied.", min_scale);
    scale = min_scale;
    /*ROS_WARN_COND(
        min_diff < 1.0,
        ...
        "optimality!");*/
    if (min_diff < 1.0) {
  	RCLCPP_WARN(rclcpp::get_logger("graphbase_getAdjacencyMatrix"),
  	"The adjacency matrix is ill conditioned. Consider removing small "
    	"details in the polygon to have even decomposition sizes. Loss of "
    	"optimality!");
}
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("graphbase_getAdjacencyMatrix"), "The minimum cost difference is: %.9f", min_diff);
  //ROS_INFO("The adjacency matrix scale is: %.9f", scale);
  RCLCPP_INFO(rclcpp::get_logger("graphbase_getAdjacencyMatrix"), "The adjacency matrix scale is: %.9f", scale);
