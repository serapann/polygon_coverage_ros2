...
bool BooleanLattice::create() {
  ...
  /*ROS_INFO_STREAM("Successfully created boolean lattice with "
                  ...
                  << " edges.");*/
  RCLCPP_INFO_STREAM(rclcpp::get_logger("BooleanLattice_create"), "Successfully created boolean lattice with " << graph_.size() << " nodes and " << edge_properties_.size() << " edges.");
...
bool BooleanLattice::addEdges() {
  if (graph_.empty()) {
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("BooleanLattice_addEdges"),"Cannot add edges to an empty graph.");
    return false;
  }
...
bool BooleanLattice::addStartNode(const NodeProperty& node_property) {
  if (!is_created_) {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("BooleanLattice_addStartNode"),"create() needs to be called first.");
    return false;
  }
...
bool BooleanLattice::addGoalNode(const NodeProperty& node_property) {
  if (!is_created_) {
    //ROS_ERROR_STREAM("create() needs to be called first.");
    RCLCPP_ERROR(rclcpp::get_logger("BooleanLattice_addGoalNode"),"create() needs to be called first.");
    return false;
  }
