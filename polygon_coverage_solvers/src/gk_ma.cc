...
const std::string kPackagePath = ament_index_cpp::get_package_share_directory(kPackageName);
...
GkMa::GkMa() {
  ...
  MonoImage* image = mono_assembly_get_image(assembly);
  solver_class_ = mono_class_from_name(image, "GkMa", "OurSolver");
  // 使用标准的C++ assert 宏来进行断言检查
  assert(solver_class_);
  // 如果要输出错误消息，可以使用 C++ 的 std::cerr
  if (!solver_class_) {
    std::cerr << "Cannot find OurSolver in assembly " 
...
bool GkMa::solve() {
  ...
  if (!solver_) {
    RCLCPP_ERROR(rclcpp::get_logger("gkma_solve"), "Solver not set.");
    return false;
    }
  ...
  if (solve == NULL || solution_at_index == NULL) {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("gkma_solve"), "Method Solve() not found.");
    RCLCPP_ERROR(rclcpp::get_logger("gkma_solve"), "Method SolutionAtIndex(int index) not found.");
    return false;
  }
  ...
  if (solution_length == NULL) {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("gkma_solve"),"Getter SolutionLength() not found.");
    return false;
  }
  MonoObject* result =
