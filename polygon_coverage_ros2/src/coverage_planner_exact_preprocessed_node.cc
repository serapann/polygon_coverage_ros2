...
int main(int argc, char** argv) {
  ...
  // Create a ROS 2 node
  auto node = std::make_shared<rclcpp::Node>("stripmap_planner_2d_node"); 
  // Create a ROS 2 node with a private namespace
  auto node_private = std::make_shared<rclcpp::Node>("stripmap_planner_2d_node", rclcpp::NodeOptions().use_intra_process_comms(true));
  // Create the coverage planner with ROS 2 interface
  auto planner = std::make_shared<polygon_coverage_planning::CoveragePlanner<
      polygon_coverage_planning::PolygonStripmapPlannerExactPreprocessed>>(node,node_private);
